...
class Cart {
				...
				if ( isset( $cart_item['subscription'] ) && $cart_item['subscription']['type'] ) {
					...
					$recurrings[]      = apply_filters(
						...
						array(
							...
							'description' => 'Next billing on: ' . $next_date,
						),
						$cart_item
					);
				}
	...
	public function add_to_cart_item_data( $cart_item_data, $product_id ) {
		...
		if ( $enabled ) :
			...
			$cart_item_data['subscription']  = apply_filters( 'subscrpt_block_cart_item_data', $subscription_data, $product, $cart_item_data );
		endif;
		return $cart_item_data;
	}
	...
	public function add_rows_order_total() {
		...
		foreach ( $cart_items as $cart_item ) {
			...
			if ( ! $product->is_type( 'variable' ) && $product->get_meta( '_subscrpt_enabled' ) ) :
				$type       = Helper::get_typos( 1, $product->get_meta( '_subscrpt_timing_option' ) );
				$price_html = wc_price( $cart_item['line_subtotal'] ) . '/ ' . $type;
				$start_date = null;
				$next_date  = Helper::next_date( 1 . ' ' . $type );
				$recurrs[]  = apply_filters(
					'subscrpt_single_recurring_item',
					array(
						'trial'      => null,
