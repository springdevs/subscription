...
class Thankyou {
    ...
    public function subscription_thank_you( $order_id ) {
        ...
        if ( ! empty( $post_meta ) && is_array( $post_meta ) && $post_meta['status'] ) {
            return;
        }
        $order_subscrpt_products  = array();
        $order_subscrpt_full_data = array();
                ...
                if ( is_array( $post_meta ) && $post_meta['enable'] ) :
                    ...
                    if ( ! empty( $post_meta['trial_time'] ) && $post_meta['trial_time'] > 0 && ! $is_renew && $has_trial ) {
                        ...
                        if ( isset( $post_meta['signup_fee'] ) ) {
                            $signup_fee = $post_meta['signup_fee'];
                        }
                        $start_date = strtotime( $trial );
                    }
                    $_subscrpt_order_general = array(
                        'order_id'            => $order_id,
                        'stats'               => 'Parent Order',
                    ...
                    if ( $unexpire_data['post'] == 0 || ! $is_renew || get_the_title( $unexpire_data['post'] ) == '' ) {
                        ...
                        do_action('subscrpt_subscription_created', $post_id, $order_id, $cart_item, $post_status);
                    }
                    ...
                    if ( $is_renew ) {
                        $_subscrpt_order_general['stats'] = 'Renew Order';
                    }
                    ...
                    <table class="woocommerce-table woocommerce-table--order-details shop_table order_details">
                        ...
                        <tr class="woocommerce-table__line-item order_item">
                            <td class="woocommerce-table__product-name product-name">
                                ...
                                <strong
                                    class="product-quantity">×&nbsp;<?php echo esc_html( $subscrpt_meta['qty'] ); ?></strong>
                            </td>
